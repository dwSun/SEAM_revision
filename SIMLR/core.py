...
class SIMLR_LARGE(object):
    ...
    def mex_multipleK(self, val, ind):
        ...
        for k in allK:
            ...
            temp0 = 0.5 * (temp[:, np.newaxis] + np.take(temp, ind))
            temp = val / temp0
            temp *= temp
            for s in sigma:
                temp1 = np.exp(-temp / 2.0 / s / s) / np.sqrt(2 * np.pi) / s / temp0
                temptemp = temp1[:, 0]
                temp1[:] = 0.5 * (temptemp[:, np.newaxis] + temptemp[ind]) - temp1
                if ismemory:
    ...
    def fit(self, X, beta=0.8):
        ...
        rr = (0.5 * (K * distX[:, K + 2] - distX[:, np.arange(1, K + 1)].sum(axis=1))).mean()
        lambdar = rr
        S0 = distX.max() - distX
        S0[:] = helper.NE_dn(S0)
